    ...
    private loadModel(position: pc.Vec3): void {
        this.app.assets.loadFromUrl('/assets/mixamo.glb', 'container', (err, asset) => {
            ...
            this.animMap = {
                ...
                start_walk: container.animations.find(a => a.name.toLowerCase().includes('animation/9'))!,
                stop_walk: container.animations.find(a => a.name.toLowerCase().includes('animation/10'))!,
                talking_phone_standing: container.animations.find(a => a.name.toLowerCase().includes('animation/11'))!,
                talking_phone_walking: container.animations.find(a => a.name.toLowerCase().includes('animation/12'))!,
                telling_secret: container.animations.find(a => a.name.toLowerCase().includes('animation/13'))!,
                texting_standing: container.animations.find(a => a.name.toLowerCase().includes('animation/14'))!,
                left_strafe_walk: container.animations.find(a => a.name.toLowerCase().includes('animation/15'))!,
                right_strafe_walk: container.animations.find(a => a.name.toLowerCase().includes('animation/16'))!,
                backward: container.animations.find(a => a.name.toLowerCase().includes('animation/17'))!,
                walkup_stairs: container.animations.find(a => a.name.toLowerCase().includes('animation/18'))!,
                texting_walking: container.animations.find(a => a.name.toLowerCase().includes('animation/19'))!,
            };
